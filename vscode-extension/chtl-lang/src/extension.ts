...
export function activate(context: vscode.ExtensionContext) {
    ...
    // Initialize compiler
    compiler = new ChtlCompiler(context);
    ...
    if (!compiler.isAvailable()) {
        vscode.window.showWarningMessage(
            'CHTL compiler not found. Some features may not work. Please install the CHTL compiler or update your settings.',
            'Open Settings'
        ).then(selection => {
    ...
    const saveWatcher = vscode.workspace.onDidSaveTextDocument(async (document) => {
        if (document.languageId === 'chtl') {
            ...
            if (config.get<boolean>('lint.enable')) {
                await lintDocument(document);
            }
            
            // Auto-compile if enabled
            if (config.get<boolean>('compiler.autoCompile')) {
...
function updateDiagnostics(uri: vscode.Uri, diagnostics: Diagnostic[]) {
...
function getChtlHoverInfo(word: string): vscode.MarkdownString | undefined {
...
function provideCompletionItems(
    ...
    if (linePrefix.endsWith('[')) {
        items.push(
            createCompletionItem('Configuration]', vscode.CompletionItemKind.Keyword, 'Global configuration'),
            createCompletionItem('Name]', vscode.CompletionItemKind.Keyword, 'Keyword aliases (inside Configuration)'),
            createCompletionItem('Import]', vscode.CompletionItemKind.Keyword, 'Import components'),
            createCompletionItem('Export]', vscode.CompletionItemKind.Keyword, 'Export components'),
            createCompletionItem('Namespace]', vscode.CompletionItemKind.Keyword, 'Namespace definition'),
            createCompletionItem('Custom]', vscode.CompletionItemKind.Keyword, 'Custom component'),
            createCompletionItem('Template]', vscode.CompletionItemKind.Keyword, 'Template component'),
            createCompletionItem('Origin]', vscode.CompletionItemKind.Keyword, 'Raw code embed'),
            createCompletionItem('Info]', vscode.CompletionItemKind.Keyword, 'Module information (CMOD only)'),
        );
    }
    ...
    if (linePrefix.endsWith('@')) {
        items.push(
            createCompletionItem('Element', vscode.CompletionItemKind.Class, 'Element component'),
            createCompletionItem('Style', vscode.CompletionItemKind.Class, 'Style group'),
            createCompletionItem('Var', vscode.CompletionItemKind.Class, 'Variable group'),
            createCompletionItem('Html', vscode.CompletionItemKind.Class, 'HTML origin'),
            createCompletionItem('JavaScript', vscode.CompletionItemKind.Class, 'JavaScript origin'),
        );
    }
    ...
    if (linePrefix.match(/^\s*\w*$/)) {
        items.push(
            createCompletionItem('inherit', vscode.CompletionItemKind.Keyword, 'Inherit from template/custom'),
            createCompletionItem('delete', vscode.CompletionItemKind.Keyword, 'Delete properties/elements'),
            createCompletionItem('insert', vscode.CompletionItemKind.Keyword, 'Insert elements'),
            createCompletionItem('except', vscode.CompletionItemKind.Keyword, 'Exclude elements'),
            createCompletionItem('from', vscode.CompletionItemKind.Keyword, 'Import/export from source'),
            createCompletionItem('as', vscode.CompletionItemKind.Keyword, 'Rename import/export'),
            createCompletionItem('style', vscode.CompletionItemKind.Keyword, 'Local style block'),
            createCompletionItem('script', vscode.CompletionItemKind.Keyword, 'Local script block'),
            createCompletionItem('text', vscode.CompletionItemKind.Keyword, 'Text content block'),
        );
    }
    ...
    if (linePrefix.includes('{{')) {
        items.push(
            createCompletionItem('&}}', vscode.CompletionItemKind.Variable, 'Current element reference'),
            createCompletionItem('.class}}', vscode.CompletionItemKind.Variable, 'Select by class'),
            createCompletionItem('#id}}', vscode.CompletionItemKind.Variable, 'Select by ID'),
            createCompletionItem('tag}}', vscode.CompletionItemKind.Variable, 'Select by tag'),
        );
    }

    if (linePrefix.includes('->')) {
        items.push(
            createCompletionItem('listen', vscode.CompletionItemKind.Method, 'Add event listeners'),
            createCompletionItem('delegate', vscode.CompletionItemKind.Method, 'Event delegation'),
            createCompletionItem('animate', vscode.CompletionItemKind.Method, 'Animate element'),
        );
    }
